...
- **云端总目录/本地总目录**：`D:\All projects\Obsidian-Assistance`
- **本目录路径**：`D:\All projects\Obsidian-Assistance\Obsidian - Backend Assistance`
- **归属定义**：本目录是云端总仓库中的 **Obsidian 后端增强目录**，由 Hermes 调用、开发、验证和推送。
- **同级前端/UI 目录**：`D:\All projects\Obsidian-Assistance\Obsidian - Front-end Assistance` 是云端总仓库中的前端增强目录，主要由 Open Design 负责 UI/界面产物。

本目录只保存 **Obsidian 后端辅助工具、流程规范、脚本模板、本地核验模块和脱敏交接文档**，不保存正式 Obsidian vault、原始课程资料、音视频转写、OCR 全文、私人笔记或同步盘内容。
